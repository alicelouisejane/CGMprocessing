...
cgmanalysis2 <- function (exerciseanalysis=TRUE,inputdirectory,outputdirectory, patientdata, outputname,
  ...
  for (f in 1:base::length(files)) {
    ...
    if(awakeorsleepor24=="awake"){
      daytime_indexes <-
        base::which(base::as.numeric(base::format(table$timestamp,"%H")) %in%
                      allhours[base::which(!(0:23 %in% (daystart-1):dayend))])
      table<-table[daytime_indexes,]
    }else if(awakeorsleepor24=="sleep"){
      nighttime_indexes <-
        base::which(base::as.numeric(base::format(table$timestamp,"%H")) %in%
                      allhours[base::which((0:23 %in% (daystart-1):dayend))])
      table<-table[nighttime_indexes,]
    }else if(awakeorsleepor24=="24"){
      table<-table
    }
    ...
    # this does not add in interpolation and only adds time that we have readings for
    totaltime<-nrow(table)*300

    # #The total time that is in the data set, splits it up by date to get true total time avoiding error with max and min
    ...
    # time$consecutive <- c(FALSE,diff(as.Date(time$date))>1)
    # insertpostionstime<-which(time$consecutive %in% TRUE)
    # if(!is.null(insertpostionstime)){
    #   #time<-time%>% tibble::add_row(,.before=c(insertpostionstime)) #missing variables means that they all will be given value of NA which is what we want
    #   totaltime <- time %>% dplyr::group_by(date) %>% dplyr::summarise(max=max(timestamp), min=min(timestamp),
    #                                                                    time=difftime(base::as.POSIXct(unlist(max)),base::as.POSIXct(unlist(min)),units="secs")) %>% summarise(sum(as.numeric(time)))
    # }else{
    #   time<-table
    ...
    cgmupload["gmi", f] <- base::round(12.71 + (4.70587* base::mean(table$sensorglucose[base::which(!is.na(table$sensorglucose))])), digits = 1)
...
require(dplyr)
    ...
    table10<-table10 %>% dplyr::group_by(date) %>% dplyr::mutate(BGover10 = dplyr::case_when(
    ...
    results10 <- data.frame(start = time10[mins10 == 1], end = time10[(mins10 == -1)[-1]]) %>%
      summarise(sum(difftime(end,start, unit="secs")))
    ...
    table13<-table13 %>% dplyr::group_by(date) %>% dplyr::mutate(BGover13 = dplyr::case_when(
      is.na(sensorglucose)~0,
    ...
    table16<-table16 %>% dplyr::group_by(date) %>% dplyr::mutate(BGover16 = dplyr::case_when(
      is.na(sensorglucose)~0,
      sensorglucose<=16~0,
      TRUE ~ 1))
    ...
    #replace where NA is with 0 anything <13 with a 0 and anything else (>=13) with a 1
    hypo<-hypo %>% dplyr::group_by(date) %>% dplyr::mutate(BGunder3 = dplyr::case_when(
      is.na(sensorglucose)~0,
      sensorglucose>=3~0,
      TRUE ~ 1))
    ...
    hypo<-hypo%>% dplyr::mutate(BGunder3 = dplyr::case_when(
      ...
      TRUE ~ BGunder3)) %>% mutate(BGunder3 = dplyr::case_when(BGunder3==2 & lag(BGunder3)==1 & lead(BGunder3)==0~1,
                                                               TRUE ~ BGunder3))
    ...
    # Find daytime
    daytime_indexes <- base::which(base::as.numeric(base::format(table$timestamp,"%H")) %in% daystart:dayend)
    daytime_sensor <- table$sensorglucose[daytime_indexes]

    #for use in AUC
    xaxis <- base::seq(from = 0, length.out = base::length(daytime_sensor),by = (interval / 60))
    #total AUC
    ...
    #add variable for baseline/6months/12months
    cgmupload["timepoint", f] <-base::strsplit(tools::file_path_sans_ext(basename(files[f])), "_")[[1]][2]
