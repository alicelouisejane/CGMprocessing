...
cleanCGM <- function(inputdirectory,
  ...
  sink("extod_clean_analysis_7daysonly.txt")
  for (f in 1:base::length(files)) {
    ...
    if (sensortype=="dexcom") {
      ...
      table$timestampfp <-base::as.POSIXct(lubridate::parse_date_time(table$timestampfp,
    ...
    if (calibrationcheck==TRUE) {
      ...
      for(i in 2:nrow(calibration)){
        ...
        #calibration$diff2[i]=abs(calibration$timestampfp[i]-calibration$timestampfp[i+1])
      }


      #     calibration2 <- calibration %>%
      #       dplyr::group_by(Date) %>%
      #       dplyr::mutate(max = max(timestampfp), min=min(timestampfp), diff=(max-min)) %>%
      #
      #
      #
      #
      #
      # calibration2$max<-base::as.POSIXct(lubridate::parse_date_time(calibration2$max,
      #                                                              dateparseorder),tz = "UTC")
      # calibration2$min<-base::as.POSIXct(lubridate::parse_date_time(calibration2$min,
      #                                                               dateparseorder),tz = "UTC")
      # calibration2$diff<-difftime(calibration2$max,calibration2$min, units="hours")
